selfcareApp.directive('treeModel', function ($compile, $parse) {
            return {
                restrict: 'A',
                    ...
                    if (treeId === "holidaytreeview") {
                        ...
                        '</ul>';
							...
							'<ul>' +
                                ...
                                '<i class="expanded" data-ng-show="node.' + nodeChildren + '.length && !node.collapsed" data-ng-click="' + treeId + '.selectNodeHead(node)"></i>' +
                                '<i class="normal" data-ng-hide="node.' + nodeChildren + '.length"></i> ' +
                                '<span ng-show="node.'+ nodeId + ' >= 0 && node.flag == 0" data-ng-class="node.selected" data-ng-click="' + treeId + '.selectNodeLabel(node); $root.tempNodeID = node.'+ nodeId +'" > {{node.' + nodeLabel + '}} ({{node.totalAmount}}) <span data-ng-click="imageViewFun(node.clientImageData)"><img src="{{node.clientImageData}}" class="avatar" width="20" ng-show="node.clientImageData"></span></span></span>' +                               
                                '<span ng-show="node.'+ nodeId + ' >= 0 && node.flag == 1" data-ng-class="node.selected" data-ng-click="' + treeId + '.selectNodeLabel(node); $root.tempNodeID = node.'+ nodeId +'" style="color:blue;"> {{node.' + nodeLabel + '}} ({{node.totalAmount}})<span data-ng-click="imageViewFun(node.clientImageData)"> </span><img src="{{node.clientImageData}}" class="avatar" width="20" ng-show="node.clientImageData"></span></span>' +                               
                                '<span ng-show="node.'+ nodeId + ' >= 0 && node.flag == 2" data-ng-class="node.selected" data-ng-click="' + treeId + '.selectNodeLabel(node); $root.tempNodeID = node.'+ nodeId +'" style="color:green;"> {{node.' + nodeLabel + '}} ({{node.totalAmount}}) <span data-ng-click="imageViewFun(node.clientImageData)"><img src="{{node.clientImageData}}" class="avatar" width="20" ng-show="node.clientImageData"></span></span>' +								
								'<span ng-show="node.'+ nodeId + ' < 0" data-ng-class="node.selected" data-ng-click="' + treeId + '.selectNodeLabel(node)" >{{node.' + nodeLabel + '}}</span>' +								
								'<div data-ng-hide="node.collapsed"  data-tree-id="' + treeId + '" data-tree-model="node.' + nodeChildren + '" data-node-id="' + nodeId + '" data-node-label="' + nodeLabel + '" data-node-children="' + nodeChildren + '"></div>' +
                                '</li>' +
                            '</ul>';
